...
#! nix-shell -i bash -p curl jq gnutar coreutils
mkdir -p capstone-work/bin capstone-work/state-node-testnet capstone-work/tarballs
pushd capstone-work
pushd tarballs
if [ ! -f cardano-node-1.29.0-macos.tar.gz ]
then
  echo "Fetching MacOS Binaries"
  curl -O -J https://hydra.iohk.io/build/7408630/download/1/cardano-node-1.29.0-macos.tar.gz
fi
...
then
  ...
  tar -zxf tarballs/cardano-node-1.29.0-macos.tar.gz -C bin/
fi
